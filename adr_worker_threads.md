# ADR: Worker Thread System Execution — Rejected

**Status:** Rejected and reverted. Do not re-attempt without first redesigning the
ECS for concurrent entity access; see Problems Encountered below.

## Overview
This ADR records an attempt to implement worker thread execution for game systems to improve performance by running non-raylib systems in parallel with the main thread. The sections below describe what was tried, not what is proposed.
