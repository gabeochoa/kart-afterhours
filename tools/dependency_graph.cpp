...
static std::vector<std::string> split_top_level_commas(const std::string &s) {
	...
	for (size_t i = 0; i < s.size(); ++i) {
		char ch = s[i];
		if (ch == '<') depth_lt++;
		else if (ch == '>') depth_lt = std::max(0, depth_lt - 1);
		else if (ch == '(') depth_paren++;
		else if (ch == ')') depth_paren = std::max(0, depth_paren - 1);
...
static std::optional<std::pair<size_t, size_t>> find_brace_block(const std::string &s, size_t start_brace) {
	if (start_brace >= s.size()) return std::nullopt;
	if (s[start_brace] != '{') {
	...
	for (size_t i = start_brace; i < s.size(); ++i) {
		if (s[i] == '{') depth++;
			...
			depth--;
			if (depth == 0) return std::make_pair(start_brace + 1, i);
		}
	}
	return std::nullopt;
}

struct StructInfo { std::string name, base_decl, base_type, body; std::vector<std::string> template_args; std::string file_path; };

static std::vector<StructInfo> find_structs_with_system_bases(const std::string &text_in, const std::string &file_path) {
	std::vector<StructInfo> out;
	std::string text = strip_comments(text_in);
	std::regex struct_re(R"(\bstruct\s+([A-Za-z_]\w*)\s*:\s*([^\{;]+)\{)");
	for (auto it = std::sregex_iterator(text.begin(), text.end(), struct_re); it != std::sregex_iterator(); ++it) {
		std::smatch m = *it;
		std::string name = m[1];
		...
		bool found = false;
		for (auto tit = std::sregex_iterator(bases.begin(), bases.end(), templ_re); tit != std::sregex_iterator(); ++tit) {
			std::smatch mm = *tit;
			std::string btype = mm[1];
			std::string simple = btype;
			size_t pos = simple.rfind("::"); if (pos != std::string::npos) simple = simple.substr(pos + 1);
			if (simple == "System" || simple == "PausableSystem" || simple == "SystemWithUIContext") {
				base_type = simple;
				std::string argstr = mm[2];
				auto args = split_top_level_commas(argstr);
				for (auto &a : args) template_args.push_back(normalize_type(a));
		...
		if (!found) continue;
		out.push_back(StructInfo{ name, base_decl, base_type, body, template_args, file_path });
	}
	return out;
}

static void parse_for_each_params(const std::string &body, SystemModel &sys) {
	std::regex fe_re(R"(for_each_with(?:_derived)?\s*\((.*?)\)\s*(?:const\s*)?(?:override\b)?)");
	for (auto it = std::sregex_iterator(body.begin(), body.end(), fe_re); it != std::sregex_iterator(); ++it) {
		std::smatch m = *it;
		std::string params = m[1];
		auto parts = split_top_level_commas(params);
		for (auto &p : parts) {
			if (p.find("float") != std::string::npos) continue;
			std::regex pm_re(R"((const\s+)?([A-Za-z_][\w:<>]*)\s*&)");
			std::smatch pm;
			if (!std::regex_search(p, pm, pm_re)) continue;
			...
			if (is_const) sys.read_components.insert(tname);
			else { sys.read_components.insert(tname); sys.write_components.insert(tname); }
		}
	}
}

static std::pair<std::unordered_map<std::string, std::string>, std::unordered_map<std::string, int>> parse_stage_orders(const std::string &text) {
	std::unordered_map<std::string, std::string> stage;
	std::unordered_map<std::string, int> order;
	struct Item { size_t pos; std::string stage; std::string token; };
	std::vector<Item> items;
	std::regex re_fixed(R"(register_fixed_update_system\s*\(\s*std::make_unique<\s*([A-Za-z_][\w:<>]*)\s*>\s*\()");
	std::regex re_update(R"(register_update_system\s*\(\s*std::make_unique<\s*([A-Za-z_][\w:<>]*)\s*>\s*\()");
	std::regex re_render(R"(register_render_system\s*\(\s*std::make_unique<\s*([A-Za-z_][\w:<>]*)\s*>\s*\()");
	for (auto it = std::sregex_iterator(text.begin(), text.end(), re_fixed); it != std::sregex_iterator(); ++it)
		items.push_back({ static_cast<size_t>((*it).position()), "fixed_update", (*it)[1] });
	for (auto it = std::sregex_iterator(text.begin(), text.end(), re_update); it != std::sregex_iterator(); ++it)
		items.push_back({ static_cast<size_t>((*it).position()), "update", (*it)[1] });
	for (auto it = std::sregex_iterator(text.begin(), text.end(), re_render); it != std::sregex_iterator(); ++it)
		items.push_back({ static_cast<size_t>((*it).position()), "render", (*it)[1] });
	std::sort(items.begin(), items.end(), [](const Item &a, const Item &b){ return a.pos < b.pos; });
	std::unordered_map<std::string, int> counters { {"fixed_update",0}, {"update",0}, {"render",0} };
	for (auto &it : items) {
		std::string tok = it.token;
		size_t p = tok.rfind("::"); if (p != std::string::npos) tok = tok.substr(p + 1);
		size_t lt = tok.find('<'); if (lt != std::string::npos) tok = tok.substr(0, lt);
		if (!stage.count(tok)) stage[tok] = it.stage;
		if (!order.count(tok)) order[tok] = counters[it.stage]++;
	}
	return { stage, order };
}
...
static std::string systems_html_template() {
	// Reuse the HTML from Python generator; [[SUMMARY_JSON]] placeholder to replace
...
int main(int argc, char** argv) {
	...
	for (int i = 1; i < argc; ++i) {
		std::string a = argv[i];
		auto next = [&](fs::path &p){ if (i + 1 < argc) { p = argv[++i]; } };
		if (a == "--src") next(src);
		else if (a == "--main") next(main_cpp);
		else if (a == "--outdir") next(outdir);
	}
	...
	for (auto const &entry : fs::recursive_directory_iterator(src)) {
		...
		if (ext != ".h" && ext != ".hpp" && ext != ".cpp" && ext != ".cc" && ext != ".cxx") continue;
		...
		for (auto &st : structs) {
			...
			for (auto &t : sm.declared_components) {
				if (!t.empty() && sm.read_components.count(t) == 0 && sm.write_components.count(t) == 0) {
					if (t != "afterhours::ui::UIContext<InputAction>") sm.read_components.insert(t);
				}
			}
			auto itS = stage.find(sm.name); if (itS != stage.end()) sm.stage = itS->second; else sm.stage = "unknown";
			auto itO = order.find(sm.name); sm.order = (itO != order.end() ? itO->second : -1);
	...
	for (auto &s : systems) {
		...
		j["stage"] = s.stage;
		j["order"] = s.order;
		std::vector<std::string> decl = s.declared_components; std::sort(decl.begin(), decl.end()); j["declared_components"] = decl;
		std::vector<std::string> reads(s.read_components.begin(), s.read_components.end()); std::sort(reads.begin(), reads.end()); j["reads"] = reads;
		std::vector<std::string> writes(s.write_components.begin(), s.write_components.end()); std::sort(writes.begin(), writes.end()); j["writes"] = writes;
	...
	std::cout << "Wrote: " << (outdir / "dependency_summary.json") << " and " << (outdir / "systems.html") << "\n";
	return 0;
}
