...
struct MultipassRenderer {
  ...
  struct PassConfig {
    ...
    PassConfig(RenderPriority p, bool clear_before = false, bool clear_after = false, raylib::Color clear_color = raylib::BLACK)
      : priority(p), clear_before(clear_before), clear_after(clear_after), clear_color(clear_color) {}
  };

  // Pre-configured render passes
  std::vector<PassConfig> pass_configs = {
    {RenderPriority::Background, true, false, raylib::SKYBLUE},  // Clear to sky color
    {RenderPriority::Entities, false, false},                     // Don't clear, render entities
    {RenderPriority::Particles, false, false},                    // Don't clear, render particles
    {RenderPriority::UI, false, false},                           // Don't clear, render UI
    {RenderPriority::PostProcess, false, false},                  // Don't clear, post-processing
  ...
  void render_pass(EntityContainer& entities, RenderPriority priority) {
    ...
    for (Entity* entity : pass_entities) {
      if (!entity->has<HasShader>()) continue;
      
      const auto& shader_comp = entity->get<HasShader>();
      if (!shader_comp.enabled) continue;
  ...
  void render_entity(const Entity& entity, const HasShader& shader_comp) {
    ...
    for (const auto& shader_type : shader_comp.shaders) {
      ...
      // The shader is now active and will be used for subsequent rendering calls
  ...
  void set_common_uniforms(ShaderType shader_type, const raylib::Shader& shader) {
    auto& shader_lib = ShaderLibrary::get();
    
    // Set time uniform if available
    int time_loc = shader_lib.get_uniform_location(shader_type, UniformLocation::Time);
    if (time_loc != -1) {
    ...
    int resolution_loc = shader_lib.get_uniform_location(shader_type, UniformLocation::Resolution);
    if (resolution_loc != -1) {
      raylib::Vector2 resolution = {static_cast<float>(raylib::GetScreenWidth()), 
                                   static_cast<float>(raylib::GetScreenHeight())};
      raylib::SetShaderValue(shader, resolution_loc, &resolution, raylib::SHADER_UNIFORM_VEC2);
    }
  }

  // Set entity-specific uniforms
  void set_entity_uniforms(ShaderType shader_type, const raylib::Shader& shader, 
                           const raylib::Color& color, float speed = 0.0f, bool is_winner = false) {
    auto& shader_lib = ShaderLibrary::get();
    
    // Set entity color uniform if available
    int color_loc = shader_lib.get_uniform_location(shader_type, UniformLocation::EntityColor);
    if (color_loc != -1) {
      raylib::Vector4 color_vec = {color.r / 255.0f, color.g / 255.0f, color.b / 255.0f, color.a / 255.0f};
      raylib::SetShaderValue(shader, color_loc, &color_vec, raylib::SHADER_UNIFORM_VEC4);
    }
    
    // Set speed uniform if available
    int speed_loc = shader_lib.get_uniform_location(shader_type, UniformLocation::Speed);
    if (speed_loc != -1) {
      raylib::SetShaderValue(shader, speed_loc, &speed, raylib::SHADER_UNIFORM_FLOAT);
    }
    
    // Set winner rainbow uniform if available
    int rainbow_loc = shader_lib.get_uniform_location(shader_type, UniformLocation::WinnerRainbow);
    if (rainbow_loc != -1) {
      float rainbow_value = is_winner ? 1.0f : 0.0f;
      raylib::SetShaderValue(shader, rainbow_loc, &rainbow_value, raylib::SHADER_UNIFORM_FLOAT);
    }
  }

  // Configure a render pass
  void configure_pass(RenderPriority priority, bool clear_before, bool clear_after, raylib::Color clear_color = raylib::BLACK) {
