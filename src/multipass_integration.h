...
private:
  void configure_render_passes() {
    ...
    renderer.configure_pass(RenderPriority::Background, true, false, raylib::SKYBLUE);
    ...
    // Configure post-processing pass to not clear (apply effects to existing render)
...
// Example system that demonstrates how to update existing entities to use multipass
...
struct MultipassUniformSystem : System<> {
  virtual void for_each_with(Entity& entity, HasShader& shader_comp, float dt) override {
    auto& renderer = MultipassRenderer::get();
    
    // Example: Update entity uniforms based on game state
    for (const auto& shader_type : shader_comp.shaders) {
      if (shader_type == ShaderType::Car || shader_type == ShaderType::CarWinner) {
...
private:
  void update_car_uniforms(Entity& entity, ShaderType shader_type) {
    ...
    // Note: This is just an example - the actual uniform setting happens in the renderer
    // This system would prepare the data that the renderer uses
  }
};
