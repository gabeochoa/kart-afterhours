...
#define log_trace(...)                                                         \
  if (static_cast<int>(LogLevel::LOG_TRACE) >= static_cast<int>(AFTER_HOURS_LOG_LEVEL))                            \
  log_me(LogLevel::LOG_TRACE, __FILE__, __LINE__, __VA_ARGS__)

#define log_info(...)                                                          \
  if (static_cast<int>(LogLevel::LOG_INFO) >= static_cast<int>(AFTER_HOURS_LOG_LEVEL))                             \
  log_me(LogLevel::LOG_INFO, __FILE__, __LINE__, __VA_ARGS__)
#define log_warn(...)                                                          \
  if (static_cast<int>(LogLevel::LOG_WARN) >= static_cast<int>(AFTER_HOURS_LOG_LEVEL))                             \
  log_me(LogLevel::LOG_WARN, __FILE__, __LINE__, __VA_ARGS__)
#define log_error(...)                                                         \
  if (static_cast<int>(LogLevel::LOG_ERROR) >= static_cast<int>(AFTER_HOURS_LOG_LEVEL))                            \
